...
class UserModel{
    ...
    function insert($password,$user,$name){
        ...
        (`id_usuario`, `nombre`, `email`, `password`,`id_rol`)
         VALUES (NULL, ?, ?, ?, ?);');
   ...
   function getAll(){
       $query = $this->db->prepare(
            'SELECT u.id_usuario, u.nombre, u.email, u.id_rol, r.rol 
            FROM usuarios AS u 
   ...
   function delete($user_id){
       $query = $this->db->prepare('DELETE FROM usuarios WHERE usuarios.id_usuario = ?');
